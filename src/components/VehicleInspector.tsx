    ...
    >
      ...
      <div className="rounded-2xl p-4 backdrop-blur-xl"
        ...
        <div className="flex items-center gap-3 mb-3">
          <div className="text-3xl p-2 rounded-xl bg-amber-950/60 border border-amber-500/30">🚛</div>
          <div>
          ...
          <div className="ml-auto px-2.5 py-1 rounded-lg text-xs font-mono font-bold bg-emerald-950/70 text-emerald-300 border border-emerald-500/40">
            DISPATCH READY
          </div>
        </div>
    ...
    >
      ...
      <div className="rounded-2xl p-3.5 backdrop-blur-xl flex flex-col gap-1.5"
        ...
        <div className="text-[10px] font-mono font-bold text-slate-400 mb-1 flex items-center justify-between border-b border-white/5 pb-1">
          ...
          <span className="text-amber-400">6 COWS ONBOARD</span>
        </div>
          ...
          <AnimatePresence key={animal.id}>
            {i < animalsDone && (
              <motion.div
                ...
                animate={{ opacity: 1, x: 0 }}
                className="flex justify-between items-center py-1.5 px-2.5 rounded-lg bg-slate-900/80 border border-white/5"
              >
          ...
          >
            <div className="rounded-2xl p-3.5 backdrop-blur-xl"
              ...
              <div className="flex justify-between items-center py-1">
                ...
                <span className="font-mono font-bold text-emerald-400">{animalLoadDemo.availableFloorArea} m²</span>
              </div>
              <div className="flex justify-between items-center pt-2 mt-1 rounded-xl px-3 py-2" style={{ background: 'rgba(22,163,74,0.15)', border: '1px solid rgba(22,163,74,0.3)' }}>
                <span className="text-xs font-bold text-emerald-200">WEIGHT & AREA SAFETY CHECK</span>
                <span className="font-bold text-xs text-emerald-400 font-mono">✓ CERTIFIED SAFE</span>
              </div>
            </div>
          </motion.div>
        )}
      ...
      <AnimatePresence>
        {showRouteCheck && (
          ...
          >
            ...
            {/* R1 — REJECTED */}
            <div className="rounded-2xl p-3 backdrop-blur-xl" style={{ background: 'rgba(38, 16, 20, 0.9)', border: '1px solid rgba(224,72,56,0.4)' }}>
              ...
              {[
                ...
                <div key={row.check} className="flex justify-between items-center py-0.5 text-xs">
                  ...
                  <span className="flex items-center gap-1 font-bold text-[11px]" style={{ color: row.result.startsWith('PASS') ? '#10b981' : '#ef4444' }}>
            ...
            {/* R3 — APPROVED */}
            <div className="rounded-2xl p-3 backdrop-blur-xl" style={{ background: 'rgba(16, 30, 22, 0.9)', border: '1px solid rgba(22,163,74,0.4)' }}>
              <div className="flex justify-between items-center mb-1.5">
                <span className="text-xs font-bold text-white">T07 + ROUTE R3 (ELEVATED BUND)</span>
                <motion.span
                  animate={{ boxShadow: ['0 0 8px rgba(22,163,74,0)', '0 0 16px rgba(22,163,74,0.4)', '0 0 8px rgba(22,163,74,0)'] }}
                  transition={{ duration: 2, repeat: Infinity }}
                  className="text-[10px] font-mono font-bold px-2 py-0.5 rounded bg-emerald-950/80 text-emerald-300 border border-emerald-500/50"
                >
                  APPROVED ✓
                </motion.span>
              </div>
